  ...
  async executeDoctorCopilot(patientId: string, chiefComplaint: string, doctorId: string) {
    return {
      aiProvenance: {
        ...
        disclaimer:
          'AI-assisted clinical decision support. Requires licensed human physician validation and signature before execution.',
      },
  ...
  async generateNursingHandover(wardId: string, nurseId: string) {
    return {
      aiProvenance: {
        ...
        disclaimer:
          'System-generated decision support. Not a clinical record until reviewed and signed by the responsible clinician.',
      },
      sbarHandover: {
        situation:
          'Male Medical Ward (MW) currently has 18 active patients (82% occupancy). 1 patient transferred from ICU overnight.',
        background:
          'Bed MW-102 recovering from acute appendectomy, vitals stable, scheduled for surgical drain removal.',
        assessment:
          'Bed MW-105 experienced mild temperature spike (38.2°C) at 04:00. Blood cultures drawn and antipyretics administered.',
        recommendation:
          'Repeat vital signs for MW-105 at 08:00; review morning CBC reports when verified by central laboratory.',
      },

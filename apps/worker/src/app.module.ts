 ...
 * `SwaggerModule`, no CORS. A worker is entered by Redis, not by a socket, so
 * everything the API needs to answer a request is dead weight in this process.
 * That separation is the point of §8.2 — "separate process from day one" — and
 * it is also why a worker can be scaled and restarted without a single request
 * being dropped.
 */
@Module({
  imports: [

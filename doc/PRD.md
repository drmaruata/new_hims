...
## 1.4 Supabase Cloud Platform Baseline

The planned backend/data platform shall use a **managed Supabase Cloud project** for PostgreSQL, Auth, Realtime and Storage, with the following boundary:

**Supabase is the data platform; NestJS remains the business/application backend.**

Supabase provides PostgreSQL, Auth, Realtime, Storage and supporting services as a hosted platform. The HIMS backend owns business rules, domain transactions, orchestration, authorization policy composition and integration workflows. Because the platform is managed, backup, disaster recovery, monitoring, patching and availability of the database are Supabase's responsibility rather than the hospital operator's; the product's own obligations shift to data residency, access control, credential management and audit.

The HIMS repository shall not run a Supabase stack in Docker. Supporting services the API calls that are not platform features — Redis, Orthanc, Gotenberg, Mailpit, ClamAV and OpenSearch — continue to run from the root Compose file on developer workstations and hospital infrastructure. Production hospital deployments run those on Linux Docker Engine or an orchestrator such as Kubernetes, with persistent storage, backup, monitoring and disaster-recovery controls.

The product shall expose patient data only through the HIMS API. Supabase's Data API is a public endpoint served to the `anon` and `authenticated` roles; the schema shall grant neither role any privilege on HIMS tables, and deployments that do not use the Data API shall disable it rather than relying on the absence of grants as the control. All database connections shall be encrypted in transit, and the API shall connect as a dedicated non-`BYPASSRLS` role. See [ADR-0004](ADR/0004-supabase-cloud-managed-platform.md).
...
The HIMS application shall be implemented with **a managed Supabase Cloud project as the primary data platform** and external/self-hosted supporting services according to deployment profile.

| Capability               | Supabase provides                                                    | Additional service required?                        | Recommended deployment                                                                    |
| ------------------------ | -------------------------------------------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Transactional PostgreSQL | Yes                                                                  | No                                                  | Supabase Cloud managed PostgreSQL; region, backup/PITR and upgrades are platform-provided |
| Authentication/JWT       | Yes (Supabase Auth)                                                  | Not initially                                       | Supabase Cloud Auth; optional enterprise IdP/SSO later                                    |
| REST Data API            | Yes (PostgREST)                                                      | No                                                  | Public endpoint: do not expose PHI through it, and disable it where unused                |
| Realtime                 | Yes                                                                  | No                                                  | Supabase Realtime for suitable UI subscriptions; durable events remain outbox-driven      |
| Object/file storage      | Yes (Storage)                                                        | Prefer durable S3-compatible backing for production | Supabase Cloud Storage; S3-compatible export where an independent copy is required        |
...
| Monitoring               | Supabase Cloud logs and metrics for the managed database             | Yes                                                 | OpenTelemetry + Prometheus/Grafana/Loki/Tempo for the application                         |
| Backup/DR/PITR           | Managed backups and point-in-time recovery                           | Verify retention meets hospital policy              | Supabase-managed PITR; hospital retains responsibility for an application-level export    |
...
The exact service list is deployment-dependent, but the following are considered **essential for a serious production HIMS even when Supabase is the data platform**: Redis/background jobs, monitoring/alerting, secrets management, secure ingress, document/PDF processing, and radiology PACS/DICOM infrastructure where RIS is deployed. Because the database is managed, high availability, backup/DR and patching of the platform are Supabase's responsibility rather than the operator's; what remains the hospital's is verifying that the project's region, backup retention and restore procedure satisfy its own continuity obligations, and maintaining an application-level export that does not depend on the platform's retention policy. (https://supabase.com/docs/guides/platform)
...
5. Supabase platform and architecture: https://supabase.com/docs/guides/platform

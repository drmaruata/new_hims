...
| Cross-tenant worker   | A worker reads or writes every tenant's rows         | A second role, `hims_platform`, holds BYPASSRLS and is given only to the two workers, with per-table grants and no `DELETE` outside the outbox |
